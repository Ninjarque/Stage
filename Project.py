...
class Project:
    ...
    def pack(self, file_path, file_name, relocate_files):
        ...
        os.makedirs(os.path.join(self.tmpdir.name, 'temp_archive.zip'))
        if relocate_files:
            self.relocate(file_path)
        js = self.to_json_component()
        js.save(os.path.join('temp_achive', self.name + '.json'))

        shutil.make_archive('temp_archive', 'zip', self.tmpdir.name)
        os.rename(os.path.join(self.tmpdir.name, 'temp_archive.zip'), file)
    ...
    def unpack(file_path):
        ...
        os.remove(temp_zip)
        return os.path.join(Project.tmpdir.name, file_path.split('/')[-1].split('.')[0])
    ...
    def load(file_path, curve_plot, bars_plot):
        ...
        js_files = []
        for file in glob.glob(unpacked_file):
